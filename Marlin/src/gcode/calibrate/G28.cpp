...
#include "../../core/debug_out.h"
  ...
  static void quick_home_xyijkuvw() {
    ...
    // Pretend the current position is 0,0
    motion.position.set(0.0f, 0.0f);
    SECONDARY_AXIS_CODE(
      motion.position.i = 0.0f,
      motion.position.j = 0.0f,
      motion.position.k = 0.0f,
      motion.position.u = 0.0f,
      motion.position.v = 0.0f,
      motion.position.w = 0.0f
    );
...
void GcodeSuite::G28() {
  ...
  reset_stepper_timeout();
    ...
    // Disable tool centerpoint control (IK) during homing for PENTA_AXIS
      ...
      const bool saved_tool_centerpoint_control = motion.tool_centerpoint_control;
    ...
    #if ALL(HAS_TOOLCHANGE, TOOLCHANGE_HOMING_USE_T0) && (DISABLED(DELTA) || ENABLED(DELTA_HOME_TO_SAFE_ZONE))
