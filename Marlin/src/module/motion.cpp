  ...
  void Motion::reset_hotend_offsets() {
    ...
    for (uint8_t e = 0; e < TOOLS; e++) {
      for (uint8_t a = 0; a < 3; a++) {
        motion.hotend_offset[e][a] = tmp[a][e];
      }
    }
    TERN_(DUAL_X_CARRIAGE, motion.hotend_offset[1].x = _MAX(X2_HOME_POS, X2_MAX_POS));
  }
#elif HOTENDS
  constexpr xyz_pos_t Motion::hotend_offset[TOOLS];
#else
...
int16_t Motion::feedrate_percentage = 100;
  ...
  abce_pos_t Motion::delta = LOGICAL_AXIS_ARRAY(0, X_HOME_POS, Y_HOME_POS, Z_HOME_POS, I_HOME_POS, J_HOME_POS, K_HOME_POS, U_HOME_POS, V_HOME_POS, W_HOME_POS);
    ...
    #endif
    bool Motion::tool_centerpoint_control = DEFAULT_TOOL_CENTERPOINT_CONTROL;
  #endif
